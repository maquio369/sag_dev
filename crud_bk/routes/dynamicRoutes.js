...
// ===== RUTAS ESPECIALES (ANTES QUE LAS GENERALES) =====

// Obtener opciones para foreign keys
router.get('/tables/:tableName/foreign-key-options/:columnName', async (req, res) => {
  try {
...
router.post('/tables/:tableName/validate', async (req, res) => {
  try {
    const { tableName } = req.params;
    const data = req.body;
...
// Búsqueda avanzada
router.post('/tables/:tableName/search', async (req, res) => {
  try {
    const { tableName } = req.params;
...
// Obtener registros de una tabla (con filtros, paginación e includes)
